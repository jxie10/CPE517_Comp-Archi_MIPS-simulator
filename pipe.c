...
void detect_hazard()
{
    ...
    //  EX hazard
    if((Reg_EXtoMEM.wb_mem >> 5) != 0
    && Reg_EXtoMEM.addr_rtd != 0
    && Reg_EXtoMEM.addr_rtd == Reg_DEtoEX.addr_rs){
        forwardA = 2;
    }
    if((Reg_EXtoMEM.wb_mem >> 5) != 0
       && Reg_EXtoMEM.addr_rtd != 0
       && Reg_EXtoMEM.addr_rtd == Reg_DEtoEX.addr_rt){
        forwardB = 2;
    }
    // MEM hazard
    if(regWrite(Reg_MEMtoWB.wb)
    && Reg_MEMtoWB.addr_rtd != 0
    && Reg_MEMtoWB.addr_rtd == Reg_DEtoEX.addr_rs){
        forwardA = 1;
    }
    if(regWrite(Reg_MEMtoWB.wb)
       && Reg_MEMtoWB.addr_rtd != 0
       && Reg_MEMtoWB.addr_rtd == Reg_DEtoEX.addr_rt){
        forwardB = 1;
    }
}
...
{
    if(memRead(Reg_DEtoEX.wb_mem) != 0
    && (Reg_DEtoEX.addr_rt == rs
    || Reg_DEtoEX.addr_rt == rt)){
        return 1;
    }
    return 0;
}
...
void pipe_stage_decode()
{
    ...
    if(op == 0 ){  //R type
    ...
    }else if (op == 0x8 || op == 0x9) {
        // addi         addiu
        Reg_DEtoEX.wb_mem = 0b00100000;
        Reg_DEtoEX.ex = 7;
    }else if(op == 0x23){   //lw
        ...
        Reg_DEtoEX.ex=1;              //ex=0001
    } else if(op == 0xf || op == 0xd){
        // lui          ori
        Reg_DEtoEX.wb_mem = 0b00100000;
        Reg_DEtoEX.ex = 7;
        Reg_DEtoEX.inum = instruction & 0x0000ffff;
    }else if(op == 31){
        //slti
        Reg_DEtoEX.wb_mem= (2<<4)|0;    //wb = 0010, m=0000
...
void pipe_stage_execute()
{
    ...
    if(regDst(Reg_DEtoEX.ex) != 0){            //RegDst=1 rd
        Reg_EXtoMEM.addr_rtd=Reg_DEtoEX.addr_rd;
    }else{
        Reg_EXtoMEM.addr_rtd=Reg_DEtoEX.addr_rt;//RegDst=0 rt
    }
...
void pipe_stage_mem()
{
    ...
    Reg_MEMtoWB.addr_rtd = Reg_EXtoMEM.addr_rtd;
    ...
    if (jump(Reg_EXtoMEM.wb_mem) != 0)  //check if jump
    {
        ...
        printf("save :%d to address: %x\n",Reg_EXtoMEM.rt_value,Reg_EXtoMEM.AluResult);
        ...
        printf("load value from addr:%x,  the value is %d\n",Reg_EXtoMEM.AluResult, Reg_MEMtoWB.rt_value);
    }
}
/*
...
void pipe_stage_wb()
{
    ...
    if(regWrite(Reg_MEMtoWB.wb) != 0)       //reg_write
    {
        if(memtoReg(Reg_MEMtoWB.wb) != 0){     // from mem, already in rt_value
            CURRENT_STATE.REGS[Reg_MEMtoWB.addr_rtd] = Reg_MEMtoWB.rt_value;
            printf("lw to reg[%d] value: %d\n",Reg_MEMtoWB.addr_rtd,Reg_MEMtoWB.rt_value);
        }
        else{
            printf("write back to register rd\n");
            CURRENT_STATE.REGS[Reg_MEMtoWB.addr_rtd]=Reg_MEMtoWB.AluResult; // from alu result
            printf("addr_rd: %d ,rd_value %d\n",Reg_MEMtoWB.addr_rtd,CURRENT_STATE.REGS[Reg_MEMtoWB.addr_rtd]);
        }
    }
    else{
